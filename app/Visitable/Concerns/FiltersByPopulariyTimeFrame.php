...
trait FiltersByPopulariyTimeFrame
{
    ...
    {
        $query->whereHas('visits', $this->betweenScopes($from, $to))
            ->withCount([
                'visits as visit_count' => $this->betweenScopes($from, $to)
            ]);
    }

    protected function betweenScopes(Carbon $from, Carbon $to)
    {
    ...
    {
        $query->popularBetween(
            now()->startOfMonth()->subMonthsWithoutOverflow(),
            now()->subMonthsWithoutOverflow()->endOfMonth()
        );
    }
